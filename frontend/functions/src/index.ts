...
function extractImageUrl(item: RSSItem): string | null {
  ...
  const debugInfo = {
    ...
    mediaThumbnail: item.media?.thumbnail?.length || 0,
    mediaContent: item.media?.content?.length || 0,
    enclosureType: item.enclosure?.type || 'none'
  };
...
// テスト用：単一フィードでデバッグ
export const testSingleFeed = functions.https.onRequest(async (req, res) => {
