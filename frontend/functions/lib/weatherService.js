...
exports.cleanupOldWeatherData = exports.saveWeatherForecasts = exports.fetchWeatherForecast = void 0;
const params_1 = require("firebase-functions/params");
// 環境変数を定義（新しい方法）
const openWeatherApiKey = (0, params_1.defineString)('OPENWEATHER_API_KEY');
// 都市IDマッピング（主要都市）
const CITY_IDS = {
    '熊本市': 1858421,
    '玉名市': 1858421,
...
const fetchWeatherForecast = async (cityName) => {
    ...
    try {
        // 5日間の天気予報を取得（3時間間隔）
        const response = await fetch(`https://api.openweathermap.org/data/2.5/forecast?id=${cityId}&appid=${apiKey}&units=metric&lang=ja`);
        if (!response.ok) {
 ...
 * 天気データをFirestoreに保存
 */
const saveWeatherForecasts = async (forecasts, db) => {
    ...
    try {
        for (const forecast of forecasts) {
            ...
            const firestoreData = Object.assign(Object.assign({}, forecast), { forecastTime: new Date(forecast.forecastTime), updatedAt: new Date(forecast.updatedAt) });
...
exports.saveWeatherForecasts = saveWeatherForecasts;
/**
 * 古い天気データを削除（7日以上前のデータ）
 */
const cleanupOldWeatherData = async (db) => {
...
exports.cleanupOldWeatherData = cleanupOldWeatherData;
