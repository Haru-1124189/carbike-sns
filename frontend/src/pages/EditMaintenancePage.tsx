...
import { MaintenancePostDoc, MaintenanceStep } from '../types';
...
export const EditMaintenancePage: React.FC<EditMaintenancePageProps> = ({ 
  post, 
  onBackClick,
  onSuccess 
}) => {
  ...
  const [carModel, setCarModel] = useState(post.carModel);
  const [mileage, setMileage] = useState(post.mileage.toString());
  const [cost, setCost] = useState(post.cost.toString());
  const [workDate, setWorkDate] = useState(post.workDate);
  const [category, setCategory] = useState(post.category);
  const [difficulty, setDifficulty] = useState(post.difficulty);
  const [totalTime, setTotalTime] = useState(post.totalTime || '');
  const [tools, setTools] = useState<string[]>(post.tools || []);
  const [newTool, setNewTool] = useState('');
  const [parts, setParts] = useState<string[]>(post.parts || []);
  const [newPart, setNewPart] = useState('');
  const [tags, setTags] = useState<string[]>(post.tags || []);
  const [newTag, setNewTag] = useState('');
  const [steps, setSteps] = useState<MaintenanceStep[]>(post.steps || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('EditMaintenancePage - Submit Debug:', {
        title,
        content,
        carModel,
      ...
      await updateMaintenancePost(post.id, maintenanceData, user.uid, displayName);
      console.log('Maintenance post updated successfully');
      alert('メンテナンス記録を更新しました！');
      onSuccess?.();
      onBackClick?.();
    } catch (err: any) {
        ...
        <header className="bg-background/80 backdrop-blur-md sticky top-[50px] z-40">
          <div className="max-w-[420px] mx-auto w-full flex items-center justify-between p-4">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-base text-text-primary font-medium">整備記録編集</span>
            </div>
            <button
