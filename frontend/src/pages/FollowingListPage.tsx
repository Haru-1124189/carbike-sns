...
export const FollowingListPage: React.FC<FollowingListPageProps> = ({
  onBackClick,
  onUserClick
}) => {
  const { user } = useAuth();
  const { following, loading, error } = useFollowData(user?.uid || '');

  const handleUserClick = (userId: string, displayName: string) => {
    console.log('FollowingListPage - handleUserClick called:', { userId, displayName });
      ...
      <div className="max-w-[420px] mx-auto">
        ...
        <header className="bg-background px-4 py-3">
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-lg font-bold text-white">フォロー中</h1>
          </div>
        </header>
        ...
        <div className="p-4">
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <div className="text-sm text-gray-400">フォロー中のユーザーがいません</div>
            </div>
          )}
        </div>
      </div>
